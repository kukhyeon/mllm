 ...
 *
 */
#ifndef TOKENIZATION_QWEN3_HPP
#define TOKENIZATION_QWEN3_HPP
...
class QWen3Tokenizer final : public BPETokenizer {
public:
    explicit QWen3Tokenizer(const std::string &vocab_file, const std::string &merge_file, bool split_special_tokens = false, bool enable_thinking = true) :
        ...
        if (enable_thinking_) {
            ...
            chat_template_end = " /think<|im_end|>\n<|im_start|>assistant\n";
        } else {
            chat_template_pre = "<|im_start|>system\nYou are a helpful assistant./no_think<|im_end|>\n<|im_start|>user\n";
            chat_template_end = " /no_think<|im_end|>\n<|im_start|>assistant\n<think>\n\n</think>";}
    }
    ...
    std::pair<bool, std::string> postprocess(std::string &text) override {
        ...
        if (text == "<|im_start|>" || text == "<|im_end|>" || text == "<unk>" || text == "</think>") return {true, ""};
